...
summary: "Verify that SSH and SSH keys are enabled on Cisco Nexus 9336C-FX2 storage switches to support CSHM and log collection."
---
= Verify the SSH configuration for Cisco Nexus 9336C-FX2 storage switches
:icons: font
:imagesdir: ../media/

[.lead]
Verify that SSH and SSH keys are enabled on Cisco Nexus 9336C-FX2 and 9336C-FX2-T storage switches to support the Ethernet switch health monitor (CSHM) and log collection features.
...
IMPORTANT: When enabling FIPS, you must change the bitcount to 256 on the switch using the command `ssh key ecdsa 256 force`. Refer to link:https://docs.netapp.com/us-en/ontap/networking/configure_network_security_using_federal_information_processing_standards_@fips@.html#enable-fips[Configure network security using FIPS^] for more details. 



.What's next?
After you've verified your SSH configuration, you
