---
permalink: switch-cisco-9336c-fx2-storage/reset-switch-9336c-storage.html
sidebar: sidebar
keywords: 9336C-FX2 reset switch to factory defaults, 9336C-FX2 switches configuration, efos
summary: "Reset Cisco Nexus 9336C-FX2 and 9336C-FX2-T storage switches to factory defaults by erasing the current switch configuration."
---
= Reset Cisco Nexus 9336C-FX2 and 9336C-FX2-T storage switches to factory defaults

:icons: font
:imagesdir: ../media/

[.lead]
Reset Cisco Nexus 9336C-FX2 and 9336C-FX2-T storage switches to factory defaults by erasing the current switch configuration and restoring the switches to their original settings.

.About this task

* You must be connected to the switch using the serial console.
* This task resets the configuration of the management network.

.Steps

. Erase the existing configuration:
+
`write erase`

+
[subs=+quotes]
----
(s2)# *write erase*

Warning: This command will erase the startup-configuration.
Do you wish to proceed anyway? (y/n) [n] *y*
----

. Reload the switch software:
+
`reload`

+
[subs=+quotes]
----
(s2)# *reload*

This command will reboot the system. (y/n)? [n] *y*
----
+

The system reboots and enters the configuration wizard. During the boot, if you receive the prompt “Abort Auto Provisioning and continue with normal setup? (yes/no)[n]”, you should respond *yes* to proceed.


.What's next
After you've reset your switches, you can link:configure-switch-overview-9336c-storage.html[reconfigure] them as needed.

// New content for AFFFASDOC-373, 2025-AUG-01
// AFFFASDOC-458, 2026-FEB-03
