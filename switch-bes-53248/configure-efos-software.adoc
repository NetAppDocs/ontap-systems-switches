...
NOTE: If you upgrade EFOS using the ONIE OS installation the configuration is reset to factory defaults and licenses are deleted. Set up the switch to return it to normal operation and then install licenses and a supported RCF.
...
The switch reboots with no configuration applied and resets to factory defaults. Follow these four steps:
.. link:configure-licenses[Install the licenses for the BES-53248 cluster switch.] 
.. link:configure-install-rcf[Install the RCF file. ]
.. link:configure-ssh[Enable SSH on the BES-53248 cluster switch.]
.. link:configure-log-collection[Enable the log collection feature.]
