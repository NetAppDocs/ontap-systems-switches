...
. Login to the switch serial console port or connect with SSH.
...
----
====
NOTE: You can also use the following protocols:
* FTP
...
* ZMODEM
+
If your software package supports secure management you can use the SSH File Transfer Protocol (SFTP) and the Secure Copy Protocol (SCP).  If FTP is used, a password is required. CLI-based file transfers using the HTTP and HTTPS protocols are supported on selected platforms where a native wget utility is available.
