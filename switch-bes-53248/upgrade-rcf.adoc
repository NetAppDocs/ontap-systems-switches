...
CAUTION: The filename must be named `reference-config.scr` when uploading and applying the RCF file or the script apply step will fail.

[start=8]
. Copy the RCF to the bootflash of switch cs2 using one of the following transfer protocols: HTTP, HTTPS, FTP, SFTP, or SCP.
+
