...
summary: "Upgrade the reference configuration file (RCF) on the Broadcom BES-53248 cluster switch after upgrading the EFOS software."
---
= Upgrade the RCF on the Broadcom BES-53248 cluster switch
:icons: font
:imagesdir: ../media/

[.lead]
Upgrade the reference configuration file (RCF) on the Broadcom BES-53248 cluster switch after upgrading the EFOS software and applying any required licenses.
...
* A console connection to the switch, required when installing the RCF from a factory-default state. 
//This requirement is optional if you have used the Knowledge Base article  https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity[How to clear configuration on a Broadcom interconnect switch while retaining remote connectivity^] to clear the configuration, beforehand.

.Suggested documentation
* Consult the switch compatibility table for the supported ONTAP and RCF versions. Refer to link:https://mysupport.netapp.com/site/info/broadcom-cluster-switch[EFOS Software download^]. Note that there can be command dependencies between the command syntax in the RCF and that found in versions of EFOS.
* Refer to the appropriate software and upgrade guides available on the link:https://www.broadcom.com/support/bes-switch[Broadcom^] site for complete documentation on the BES-53248 switch upgrade and downgrade procedures.
...
* The examples in this procedure use four nodes. These nodes use two 10GbE cluster interconnect ports `e0a` and `e0b`. Refer to the link:https://hwu.netapp.com/Home/Index[Hardware Universe^] to verify the correct cluster ports on your platforms.
...
You use the switch CLI and the ONTAP CLI (as indicated in the following steps) for port configurations. You configure ports on the first switch (cs2) and then repeat these steps for the second switch (cs1).
...
. Note the following: 
.. If additional port licenses have been installed on the switch, you must modify the RCF to configure the additional licensed ports. Refer to link:configure-licenses.html#activate-newly-licensed-ports[Activate newly licensed ports] for details. However, when you upgrade to RCF 1.12 or later, the modifications are no longer needed because all interfaces are now pre-configured. 
...
[start=8]
. Copy the RCF to the switch cs2 using one of the following transfer protocols: HTTP, HTTPS, FTP, SFTP, or SCP.
+
...
[start=20]
. [[step20]]Repeat steps 1 to 19 on switch cs1. 
...
----
+
For further details, refer to link:https://docs.netapp.com/us-en/ontap/networking/revert_a_lif_to_its_home_port.html[Revert a LIF to its home port].
...
.What's next?
After you've upgraded your RCF, you can link:configure-ssh.html[enable SSH for log collection].
...
// AFFFASDOC-396, 2025-OCT-16
// GH issue #315, 2025-DEC-18
// GH issue #321, 2026-JAN-28
// AFFFASDOC-457, 2026-FEB-04
// AFFFASDOC-473, 2026-MAR-16
// AFFFASDOC-480, 2026-APR-15 - further updates for this re-opened ticket, 2026-JUN-02
// GH issue #339, 2026-MAY-02
// GH issue #341, 2026-JUN-09