...
. Note the following: 
.. If additional port licenses have been installed on the switch, you must modify the RCF to configure the additional licensed ports. See link:configure-licenses.html#activate-newly-licensed-ports[Activate newly licensed ports] for details. However, when upgrading to RCF 1.12 or later the edits are no longer needed as all interfaces are now pre-configured. 
