...
summary: "Install the reference configuration file (RCF) and license file on the Broadcom BES-53248 cluster switch after completing initial configuration."
---
= Install the RCF and license file on the Broadcom BES-53248 cluster switch
:icons: font
:imagesdir: ../media/

[.lead]
Install the reference configuration file (RCF) and license file together on the Broadcom BES-53248 cluster switch. Beginning with EFOS 3.12.0.1, you can install both files after completing the initial switch configuration. 

CAUTION: All ports are configured when you install the RCF, but you need to install your license to activate the configured ports. 
...
* The current RCF, available from the link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab[Broadcom Cluster Switches^] page.
...
. Copy the RCF to the bootflash of switch cs2 using one of the following transfer protocols: FTP, HTTP, TFTP, SFTP, or SCP.
+
This example shows HTTP being used to copy an RCF to the bootflash on switch cs2:
+
.Show example
[%collapsible]
====
...
. Examine the banner output from the `show clibanner` command. You must read and follow these instructions to verify the proper configuration and operation of the switch. 
+
.Show example
[%collapsible]
====
...
. On the switch, verify that your changes have been made:
+
[source,cli]
----
show running-config
...
[start=21]
. [[step21]]On cluster switch cs1, shut down the ports connected to the cluster ports of the nodes.
+
...
.What's next?
After you've installed the RCF and license file, you can link:configure-ssh.html[enable SSH].

// New content for AFFFASDOC-277, 2024-NOV-04
// Typo fix, 2024-NOV-12
