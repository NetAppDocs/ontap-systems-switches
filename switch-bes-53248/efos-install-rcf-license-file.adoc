...
---
= Install the Reference Configuration File (RCF) and license file
:icons: font
:imagesdir: ../media/

[.lead]
Beginning with EFOS 3.12.0.1, you can install the Reference Configuration File (RCF) and license file after configuring the BES-53248 cluster switch. 

CAUTION: Since all ports are configured when you install the RCF, you should then install your license to activate the configured ports. 
...
[subs=+quotes]
----
(cs2)# *copy http://172.19.2.1/x/eng,tmp/src/files/RCFs/BES-53248-RCF-v1.12-Cluster-HA.txt nvram:reference-config*

Mode........................................... HTTP
Set Server IP.................................. 172.19.2.1
Path........................................... x/eng,tmp/src/files/RCFs/
...
[subs=+quotes]
----
(cs2)# *copy http://172.19.2.1/x/eng,tmp/src/files/BES-53248-LIC.dat nvram:license-key 1*
Mode........................................... HTTP
Set Server IP.................................. 172.19.2.1
Path........................................... x/eng,tmp/src/files/
...
class-map match-any HA
match ip dscp 40
match cos 5
exit

class-map match-any CLUSTER
match ip dscp 0
match cos 0
exit
!

! Policy map for Cluster/HA/RDMA traffic for all interface speeds

policy-map CLUSTER in
class RDMA
assign-queue 2
...
! Policy map and Class map definitions for 25GbE interfaces
!
!policy-map WRED_25G in
!class HA
!assign-queue 5
!police-simple rate-percent 60 64 conform-action transmit violate-action transmit
!exit
...
!exit
!
! Policy Map and Class MAP defintions for 100GbE interfaces when supported
!
!policy-map CLUSTER in
!class HA
!assign-queue 5
...
!exit
!
! Map dot1p priority values to CoS
!
...
!
! identify WRED thresholds and enable ECN
!
random-detect queue-parms 0 min-thresh 80 90 95 97 max-thresh 100 100 100 100 drop-prob 10 10 10 10 ecn
random-detect queue-parms 2 min-thresh 90 70 50 50 max-thresh 100 100 100 100 drop-prob 3 5 10 50 ecn
random-detect queue-parms 5 min-thresh 90 70 50 50 max-thresh 100 100 100 100 drop-prob 3 5 10 50 ecn
cos-queue min-bandwidth 28 0 28 0 0 43 0 0
cos-queue random-detect 0 2 5

!cos-queue min-bandwidth 40 0 0 0 0 60 0 0
...
exit
!
port-channel name 1/64 RCF-Version
!
interface 0/1-0/16
no shutdown
...
exit
!
! Interface 1/64 is reserved. The description field is being used to identify
! the version of this file when viewing output of the "show running-config" command.
! Do not delete or use this port for connections.
interface 1/64
description "RCF BES-53248 v1.12 1-CLUSTER"
exit
!
!
!===== Additional Node Ports activated with Licenses ===
!Instructions:
! (1) Apply licenses purchased to use Ports 17 - 54
...
!
! IMPORTANT NOTES
! In certain conditions, BES-53248 may not be able to auto-negotiate port
...
!
! 8-port 10/25GbE node port license block
!
interface 0/17-0/24
no shutdown
description "10/25GbE Node Port"
...
// New content for AFFFASDOC-277, 2024-NOV-4
