...
See link:configure-health-monitor.html[Install the CSHM configuration file] and link:configure-log-collection.html[Enable the log collection feature] for the steps required to enable cluster health switch log collection used for collecting switch-related log files.
