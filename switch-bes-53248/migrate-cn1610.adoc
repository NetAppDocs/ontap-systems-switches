...
* Some of the ports on BES-53248 switches are configured to run at 10GbE or 40GbE.
...
. Verify that the cluster LIFs have migrated to the ports hosted on cluster switch CL2. This might take a few seconds.
+
`network interface show -vserver Cluster`
+
.Show example
...
+
IMPORTANT: To enable the log collection feature, you must be running ONTAP 9.12.1 and later and EFOS 3.8.0.2 and later.
+
