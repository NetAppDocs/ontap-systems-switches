...
* Some of the ports on BES-53248 switches are configured to run at 10GbE or40 GbE.
* The 10GbE and 40GbE connectivity from nodes to BES-53248 cluster switches have been planned, migrated, and documented.
...
* BES-53248: ports 0/1-0/16 (10/25GbE)
+
...
. On the new switches, confirm that the ISL is cabled and healthy between the switches cs1 and cs2:
+
...
. Move all cluster node connection cables from the old CS2 switch to the new cs2 switch.
...
. Confirm the switch port connections from switch cs2's perspective using commands:
+
[subs=+quotes]
----
...
. Confirm the switch port connections from switch cs1's perspective using commands:
+
[subs=+quotes]
----
...
--
For ONTAP 9.4 and earlier, use the command: `system cluster-switch delete -device _device-name_`
----
...
.ONTAP 9.8 and later
--
Beginning with ONTAP 9.8, enable the Ethernet switch health monitor log collection feature for collecting switch-related log files, using the following two commands: `system switch ethernet log setup-password` and `system switch ethernet log enable-collection`
...
// Updates for AFFFASDOC-73, 2023-JUN-28