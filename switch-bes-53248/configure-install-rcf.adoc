...
summary: "Install the reference configuration file (RCF) on the Broadcom BES-53248 cluster switch after completing initial configuration and applying any required licenses."
---
= Install the RCF on the Broadcom BES-53248 cluster switch
:icons: font
:imagesdir: ../media/

[.lead]
Install the reference configuration file (RCF) on the Broadcom BES-53248 cluster switch after completing the initial configuration and applying any required licenses. The RCF defines port assignments, VLANs, and network settings for cluster operation.
...
* The current RCF file, available from the link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab[Broadcom Cluster Switches^] page.
...
* The examples in this procedure use four nodes. These nodes use two 10GbE cluster interconnect ports `e0a` and `e0b`. Refer to the the link:https://hwu.netapp.com/Home/Index[Hardware Universe^] to verify the correct cluster ports on your platforms.
...
. Note the following:
.. If additional port licenses have been installed on the switch, you must modify the RCF to configure the additional licensed ports. Refer to link:configure-licenses.html#activate-newly-licensed-ports[Activate newly licensed ports] for details. 
...
--
[start=8]
. Copy the RCF to the bootflash of switch cs2 using one of the following transfer protocols: HTTP, HTTPS, FTP, TFTP, SFTP, or SCP.
+
...
.What's next?
After you've installed the RCF, you can link:configure-ssh.html[enable SSH].
...
// Updates for AFFFASDOC-277, 2024-NOV-04
// Updates for GH issue #247, 2025-MAR-14
// Removed .scr for reference-config, 2025-MAY-14
// Updates for AFFFASDOC-305, 2025-JUNE-25
