...
* A console connection to the switch, required when installing the RCF from a factory-default state. This requirement is optional if you have used the https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity[KB:How to clear configuration on a Broadcom interconnect switch while retaining remote connectivity^] to clear the configuration, beforehand.
...
.ONTAP 9.8 and later
--
On ONTAP 9.8 and later, use the command: `system switch ethernet show -is-monitoring-enabled-operational true`
...
--
On ONTAP 9.7 and earlier, use the command: `system cluster-switch show -is-monitoring-enabled-operational true`
...
.. SSH into the switch.
+
Only proceed when all the cluster LIFs have been removed from ports on the switch and the switch is prepared to have the configuration cleared.
...
[start=17]
. [[step17]]On cluster switch cs1, shut down the ports connected to the cluster ports of the nodes.
+
