...
====
+
For EFOS version 3.10.0.3 and later, use the command `ntp server`. 
+
`ntp server`
+
.Show example
...
====
 ...
 .. Configure the time manually if you did not use `ntp server` to configure the time in the previous step.
+
`clock`
+
.Show example
