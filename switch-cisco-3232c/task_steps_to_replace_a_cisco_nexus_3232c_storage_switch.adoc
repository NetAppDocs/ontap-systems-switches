...
sidebar: sidebar
keywords: storage switches,replacing a Cisco Nexus 3232C non-disruptively
...
* *Confirm the switch to be replaced is S2 (<<STEP_81AB0CB892F14435A7528A4697D3BCE8,Steps 1-5>>)*
* *Disconnect the cables from switch S2 (<<STEP_A623526091004806A9CFBA980CF24099,Step 6>>)*
* *Reconnect the cables to switch NS2 (<<STEP_2CD3C8B5B1E045A2B0BE595A0BDF59B6,Step 7>>)*
* *Verify all device configurations on switch NS2 (<<STEP_2CD3C8B5B1E045A2B0BE595A0BDF59B6,Steps 8-10>>)*
...
. Check on the health status of the storage node ports to make sure that there is connection to storage switch S1: `storage port show -port-type ENET`
+
----
...
. Verify that storage switch S1 is available: `network device-discovery show`
+
----
...
. Run the `show lldp neighbors` command on the working switch to confirm that you can see both nodes and all shelves: `show lldp neighbors`
+
----
...
. Verify the shelf ports in the storage system: `storage shelf port show -fields remote-device,remote-port`
+
----
...
. Recheck the health status of the storage node ports: `storage port show -port-type ENET`
+
----
storage::*> **storage port show -port-type ENET**
                                      Speed                       VLAN
...
. Verify that both switches are available: `network device-discovery show`
+
----
storage::*> **network device-discovery show**
Node/       Local  Discovered
