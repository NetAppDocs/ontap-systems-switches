---
permalink: switch-cisco-3232c/task-how-to-migrate-from-a-two-node-switchless-cluster-to-a-cluster-with-cisco-nexus-3232C-cluster-switches.html
sidebar: sidebar
...
.Before you begin
* Make sure you have available ports for node connections
+
The cluster switches use the Inter-Switch Link (ISL) ports e1/31-32.
-
* Make sure you have appropriate cables for cluster connections:
...
* *I. Display and migrate physical and logical ports (Steps <<joyce,1>>-<<beckett,10>>)*
* *II. Shut down the reassigned LIFs and disconnect the cables (Steps <<casey,11>>-<<heaney,14)>>)*
* *III. Enable the cluster ports (Steps <<yeats,15>>-<<friel,20>>)*
* *IV. Enable the reassigned LIFs (Steps <<wilde,21>>-<<swift,33>>)*
...
. [[beckett]]Migrate the n1_clus1 and n2_clus1 LIFs to the physical ports of their destination nodes:
+
...
. [[casey]]Verify the cluster interfaces have successfully migrated:
+
`network interface show -role cluster`
+
...
. [[heaney]]Disconnect the cable from e4a on node n1.
+
...
. [[yeats]]Disconnect the cable from e4a on node n2.
+
...
. [[yeats]]Verify that the clusters are up on both nodes:
+
`network port show -role cluster`
+
----
...
. [[friel]]Verify that all the LIFs are now reverted to their home ports:
+
`network interface show -role cluster`
+
...
. [[wilde]]Display the cluster port connectivity on each node:
+
`network device-discovery show`
+
----
...
. [[swift]]Display discovered and monitored cluster switches:
+
