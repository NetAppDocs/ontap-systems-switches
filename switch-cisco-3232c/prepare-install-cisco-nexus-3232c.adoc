...
summary: "Prepare Cisco Nexus 3232C cluster switches before installing NX-OS software and the reference configuration file (RCF)."
---
= Prepare to install NX-OS software and RCF on Cisco Nexus 3232C cluster switches
:icons: font
:imagesdir: ../media/

[.lead]
Prepare Cisco Nexus 3232C cluster switches before installing NX-OS software and the reference configuration file (RCF). NX-OS and the RCF must be installed before the switch can operate in an ONTAP cluster.

.About the examples

The examples in this procedure use two nodes. These nodes use two 10GbE cluster interconnect ports `e0a` and `e0b`.

Refer to the link:https://hwu.netapp.com/SWITCH/INDEX[Hardware Universe^] to verify the correct cluster ports on your platforms. Refer to link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU[What additional information do I need to install my equipment that is not in HWU?^] for more information about switch installation requirements.
...
After you've prepared to install the NX-OS software and RCF, you can  link:install-nx-os-software-3232c.html[install the NX-OS software].
