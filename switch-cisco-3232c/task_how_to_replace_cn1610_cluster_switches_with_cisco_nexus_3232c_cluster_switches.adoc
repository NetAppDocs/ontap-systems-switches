...
* I. Replace cluster switch CL2 with C2 (Steps <<one,1>>-<<twentytwo,22>>)
* II. Replace cluster switch CL1 with C1 (Steps <<twentythree,23>>-<<forty,40>>)
...
. [[twentytwo]]Migrate the LIFs that are associated with the first CN1610 switch CL1: `network interface migrate -vserver cluster -lif _lif-name_ -source-node _node-name_`
+
...
. [[twentythree]]Verify the cluster's status: `network interface show -role cluster`
+
...
. [[forty]]Enable the cluster switch health monitor log collection feature for collecting switch-related log files: `system cluster-switch log setup-password`
+
