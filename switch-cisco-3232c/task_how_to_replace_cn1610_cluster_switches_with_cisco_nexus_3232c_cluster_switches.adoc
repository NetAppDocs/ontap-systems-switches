...
sidebar: sidebar
keywords: Cisco Nexus 3232C cluster switches,Replacing CN1610 cluster switches,Cisco CN1610 switches,Replacing with Cisco 3232C cluster switches
...
---
= How to migrate a CN1610 cluster switch to a Cisco Nexus 3232C cluster switch
:icons: font
:imagesdir: ../media/
...
The following list describes the stages you must complete when changing the cluster switches:

* I. Replace cluster switch CL2 with C2 (<<STEP_D36309534CD94CBCA3C457CB1CC06EED,Steps 1-22>>)
* II. Replace cluster switch CL1 with C1 (<<STEP_42A578F840AD481CA55C65F0D1BF33AE,Steps 23-40>>)

The examples in this procedure use four nodes: Two nodes use four 10 GbE cluster interconnect ports: e0a, e0b, e0c, and e0d. The other two nodes use two 40 GbE cluster interconnect fiber cables: e4a and e4e. The _Hardware Universe_ has information about the cluster fiber cables on your platforms.
...
. Determine the administrative or operational status for each cluster interface.
 .. Display the cluster network port attributes: `network port show -role cluster`
+
----
...
You should prepare both switches at this time. If you need to upgrade the RCF and image, you must complete the following procedure:
 ...
 .. See the _Cisco Ethernet Switch_ page on the NetApp Support Site.
+
http://support.netapp.com/NOW/download/software/cm_switches/[Cisco Ethernet Switch]
...
. Migrate the LIFs associated with the second CN1610 switch that you plan to replace: `network interface migrate -verser cluster -lif lif-name -source-node source-node-name destination-node destination-node-name -destination-port destination-port-name`
+
...
+
This configuration matches with the ISL configuration on the 3232C switch C2 when the ISLs are brought up on both switches in <<STEP_294C669B3E3C4A89A6846FC9DC4DAAF2,Step 10>>.
+
For more information on Cisco commands, see the guides listed in the https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-command-reference-list.html[Cisco Nexus 3000 Series NX-OS Command References].
+
...
. Verify that the ISLs are `up` on the 3232C switch C2: `show port-channel summary`
+
For more information on Cisco commands, see the guides listed in the https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-command-reference-list.html[Cisco Nexus 3000 Series NX-OS Command References].
+
...
. Bring up all of the cluster interconnect ports that are connected to the 3232C switch C2 on all of the nodes: `network port modify -node node-name -port port-name -up-admin true`
+
...
. Revert all of the migrated cluster interconnect LIFs that are connected to C2 on all of the nodes: `network interface revert -vserver cluster -lif lif-name`
+
----
...
. Verify that all of the cluster interconnect ports are reverted to their home ports: `network interface show -role cluster`
+
...
. Verify that all of the cluster ports are connected: `network port show -role cluster`
+
...
. Migrate the LIFs that are associated with the first CN1610 switch CL1: `network interface migrate -vserver cluster -lif lif-name -source-node node-name`
+
...
. Verify the cluster's status: `network interface show -role cluster`
+
...
. Shut down the node ports that are connected to CL1 on all of the nodes: `network port modify -node node-name -port port-name -up-admin false`
+
...
. Bring up all of the cluster interconnect ports connected to the new 3232C switch C1 on all of the nodes: `network port modify -node node-name -port port-name -up-admin true`
+
...
. Verify the status of the cluster node port: `network port show -role cluster`
+
...
. Revert all of the migrated cluster interconnect LIFs that were originally connected to C1 on all of the nodes: `network interface revert -server cluster -lif lif-name`
+
You must migrate each LIF individually as shown in the following example:
+
----
...
. Verify that the interface is now home: `network interface show -role cluster`
+
...
. Ping the remote cluster interfaces and then perform a remote procedure call server check: `cluster ping-cluster -node host-name`
+
The following example shows node n1 being pinged and the RPC status indicated afterward:
+
----
...
----
. Remove the replaced CN1610 switches if they are not automatically removed: `system cluster-switch delete -device switch-name`
+
