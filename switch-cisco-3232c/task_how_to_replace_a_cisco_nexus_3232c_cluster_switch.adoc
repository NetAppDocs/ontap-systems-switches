...
* *Display and migrate the cluster ports to switch C2 (<<STEP_96844C9BB1374885990332F452D1CAB8,Steps 1-7>>)*
* *Reconnect ISL cables from switch CL2 to switch C2, then migrate ISLs to switch CL1 and C2 (<<STEP_EF7BDC848494485184C4BDC92EEC10A9,Steps 8-14>>)*
* *Revert all LIFs to originally assigned ports (<<STEP_F23EB84BF5EF4366A56FD307D88B53DB,Steps 15-18>>)*
* *Verify all ports and LIF are correctly migrated (<<STEP_F80703A966004F608441DBC2441FEBC5,Steps 19-21>>)*
...
----
 ...
 .. Display information about the logical interfaces (LIFs): `network interface show -role cluster`
+
----
...
----
 ...
 .. Display the discovered cluster switches: `system cluster-switch show`
+
...
. Migrate the cluster LIFs to the physical node ports connected to the replacement switch C2: `network interface migrate -vserver Cluster -lif lif-name -source-node node-name -destination-node node-name -destination-port port-name`
+
...
. Verify the status of the cluster ports and their home designations: `network interface show -role cluster`
+
----
cluster::*> network interface show -role cluster
...
. Shut down the cluster interconnect ports that are physically connected to the original switch CL2: `network port modify -node node-name -port port-name -up-admin false`
+
...
. On all nodes, bring up all the cluster interconnect ports connected to the replacement switch C2: `network port modify -node node-name -port port-name -up-admin true`
+
----
...
. Revert all the migrated cluster interconnect LIFs on all the nodes: `network interface revert -vserver cluster -lif lif-name`
+
...
. Verify that the cluster interconnect ports are now reverted to their home: `network interface show`
+
...
. Verify that the cluster ports are connected: `network port show -role cluster`
+
----
cluster::*> network port show –role cluster
  (network port show)
...
. Delete the replaced cluster switch CL2 if it has not been removed automatically: `system cluster-switch delete -device cluster-switch-name`
