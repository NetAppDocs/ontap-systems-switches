...
summary: "Install the reference configuration file (RCF) on Cisco Nexus 92300YC cluster switches after initial setup."
---
= Install the RCF on Cisco Nexus 92300YC cluster switches
:icons: font
:imagesdir: ../media/

[.lead]
Install the reference configuration file (RCF) on Cisco Nexus 92300YC cluster switches after initial setup. The RCF defines port assignments and network settings required for cluster operation.
...
+
Refer to the link:https://hwu.netapp.com/Home/Index[Hardware Universe^] for the actual cluster ports supported on your platform. Refer to link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU[What additional information do I need to install my equipment that is not in HWU?^] for more information about switch installation requirements.
...
* The examples in this procedure use two nodes. These nodes use two 10GbE cluster interconnect ports `e0a` and `e0b`. Refer to the link:https://hwu.netapp.com/SWITCH/INDEX[Hardware Universe^] to verify the correct cluster ports on your platforms.
