...
summary: "Migrate an existing two-node switchless cluster to a switched cluster environment using Cisco Nexus 92300YC cluster switches."
---
= Migrate to a two-node switched cluster using Cisco Nexus 92300YC cluster switches
:icons: font
:imagesdir: ../media/

[.lead]
Migrate an existing two-node switchless cluster to a switched cluster environment using Cisco Nexus 92300YC cluster switches. This nondisruptive migration enables you to scale the cluster beyond two nodes.
...
NOTE: After your migration completes, you might need to install the required configuration file to support the Cluster Switch Health Monitor (CSHM) for 92300YC cluster switches. Refer to
...
+
The link:https://hwu.netapp.com/SWITCH/INDEX[Hardware Universe - Switches^] contains more information about cabling.
