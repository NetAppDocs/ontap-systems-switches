...
* Cluster switch CL2 is to be replaced by C2 (<<step1_replace3132q,Steps 1>> -<<step21_replace3132q, 21>>)
...
. [[step1_replace3132q]]If AutoSupport is enabled on this cluster, suppress automatic case creation by invoking an AutoSupport message:
+
...
. [[step21_replace3132q]]Remove the replaced Nexus 3132Q-V switch, if it is not already removed automatically:
+
