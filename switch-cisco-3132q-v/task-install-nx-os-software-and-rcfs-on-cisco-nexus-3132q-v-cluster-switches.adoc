...
----
+
NOTE: Verify too that the Cluster LIFs have reverted back to their home ports. If this is not the case, revert them manually using the command: `network interface revert -vserver vserver_name -lif lif_name`
