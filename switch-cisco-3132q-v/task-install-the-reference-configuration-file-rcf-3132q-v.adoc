...
----
+
NOTE: Verify that auto-revert is enabled. Verify too that the LIFs have reverted back to their home ports. If this is not the case, revert them manually using the command: `network interface revert -vserver _vserver_name_ -lif _lif_name_`
