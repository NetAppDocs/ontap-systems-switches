...
The examples in this procedure use four nodes: Two nodes use four 10 GbE cluster interconnect ports: e0a, e0b, e0c, and e0d. The other two nodes use two 40/100 GbE cluster interconnect fiber cables: e4a and e4e. The link:https://hwu.netapp.com/[Hardware Universe^] has information about the cluster fiber cables on your platforms.
...
* Cluster switch CL2 to be replaced by C2 (<<step2_replacecn1610,Steps 2>> - <<step22_replacecn1610, 22>>)
...
* Cluster switch CL1 to be replaced by C1 (<<step23_replacecn1610,Steps 23>> - <<step43_replacecn1610, 43>>)
...
. [[step2_replacecn1610]]Display information about the devices in your configuration: `network device-discovery show`
+
...
----
+
NOTE: For ONTAP 9.8 and later, use the command `system ethernet switch show -is-monitoring-enabled-operational true`.
...
[subs=+quotes]
----
cluster::*> *cluster ping-cluster -node local*
Host is n1
...
. [[step22_replacecn1610]]Ping the remote cluster interfaces and then perform a remote procedure call server check: `cluster ping-cluster`
+
The following example shows how to ping the remote cluster interfaces:
+
[subs=+quotes]
...
. [[step23_replacecn1610]]On each node in the cluster, migrate the interfaces that are associated with the first CN1610 switch CL1, to be replaced: `network interface migrate`
+
...
----
+
NOTE: Verify that auto-revert is enabled. Verify too that the Cluster LIFs have reverted back to their home ports. If this is not the case, revert them manually using the command: `network interface revert -vserver _vserver_name_ -lif _lif_name_`
