...
----
+
NOTE: Verify that auto-revert is enabled. If it is not enabled, enable it manually using the command: `network interface revert -vserver _vserver_name_ -lif _lif_name_`
