...
. Reboot switch cs2. You can ignore the "cluster ports down" events reported on the nodes while the switch reboots and the `% Invalid command at '^' marker` output. 
+
.Show example 
[%collapsible]
====
