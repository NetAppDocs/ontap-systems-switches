...
While this procedure is being followed, an operational inter-switch link (ISL) between the switches is not required. This is important because changes between RCF versions can temporarily affect ISL connectivity. To ensure nondisruptive cluster operations, cluster LIFs are all migrated to the operational partner switch while performing the steps on the target switch.
