...
[.lead]
SNMP is used to monitor the switches. Monitoring by SNMPv3 is configured by following this procedure. The Ethernet Switch Health Monitor (CSHM) utilizes SNMP to monitor the health and performance of cluster and storage switches. By default, SNMPv2c is configured automatically through the Reference Configuration File (RCF).
