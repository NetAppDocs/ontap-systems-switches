...
IMPORTANT: If you enable FIPS mode, you must complete the following: 1. Regenerate ssh keys on the switch, as per vendor instructions. 2. Regenerate ssh keys on the ONTAP side using `debug system regenerate-systemshell-key-pair` 3. Re-run log collection setup routine using `system switch ethernet log setup-password`
