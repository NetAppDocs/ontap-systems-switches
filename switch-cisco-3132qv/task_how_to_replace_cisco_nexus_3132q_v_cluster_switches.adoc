---
permalink: switch-cisco-3132qv/task_how_to_replace_cisco_nexus_3132q_v_cluster_switches.html
sidebar: sidebar
keywords: 
...
+
The _Cisco Ethernet Switches_ page has the latest RCF and NX-OS versions on your switches.
...
The examples in this procedure use four nodes: Two nodes use four 10 GB cluster interconnect ports: e0a, e0b, e0c, and e0d. The other two nodes use two 40 GB cluster interconnect ports: e4a and e4e. See the _Hardware Universe_ for the actual cluster ports on your platforms.
...
----
 ...
 .. Display the information on the discovered cluster switches: `system cluster-switch show`
+
----
...
You must prepare the replacement switch at this time. If you need to upgrade the RCF and image, you must follow these steps:
 ...
 .. On the NetApp Support Site, go to the following location: http://support.netapp.com/NOW/download/software/cm_switches/[Cisco Ethernet Switch]
...
+
http://mysupport.netapp.com/NOW/download/software/sanswitch/fcp/Cisco/netapp_cnmn/download.shtml

. Migrate the LIFs associated to the cluster ports connected to switch C2: `network interface migrate`
+
...
. Verify cluster's health: `network interface show`
+
----
cluster::*> network interface show -role cluster
       (network interface show)
...
. Shut down the ports 1/31 and 1/32 on CL1, and the active Nexus 3132Q-V switch: `shutdown`
+
...
. Verify that the ISLs are up on CL1: `show port-channel`
+
...
. Verify that the ISLs are up on C2: `show port-channel summary`
+
...
. For all nodes, revert all of the migrated cluster interconnect LIFs: `network interface revert`
+
----
...
. Verify that the cluster ports are connected: `network port show`
+
----
cluster::*> network port show –role cluster
  (network port show)
...
----
. Remove the replaced Nexus 3132Q-V switch, if it is not already removed automatically: `system cluster-switch delete`
+
----
