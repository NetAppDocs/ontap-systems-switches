...
. Display the list of neighboring devices on the switch: `show cdp neighbors`
+
----
...
. Migrate the clus1 interface to the physical port hosting clus2: `network interface migrate`
+
...
. Verify the cluster interfaces migration: `network interface show`
+
----
...
. Shut down cluster ports clus1 LIF on both nodes: `network port modify`
+
----
...
. Enable the first cluster port, e4a, on each node: `network port modify`
+
----
...
. Verify that the clusters are up on both nodes: `network port show`
+
----
cluster::*> network port show –role cluster
  (network port show)
...
. Verify that all of the cluster interconnect ports are now reverted to their home ports: `network interface show`
+
...
. On the console of each node, migrate clus2 to port e4a: `network interface migrate`
+
----
...
. Display the cluster switch port numbers each cluster port is connected to on each node: `network device-discovery show`
+
----
	cluster::*> network device-discovery show
            Local  Discovered
...
. Disable the two-node switchless configuration settings on any node: `network options switchless-cluster`
+
----
