...
summary: "Migrate from older Cisco cluster switches to NVIDIA SN2100 cluster switches without disrupting cluster operation."
---
= Migrate from a Cisco cluster switch to an NVIDIA SN2100 cluster switch
:icons: font
:imagesdir: ../media/

[.lead]
Migrate from Cisco cluster switches to NVIDIA SN2100 cluster switches without disrupting cluster operation.

== Review requirements

You must be aware of certain configuration information, port connections and cabling requirements when you are replacing some older Cisco cluster switches with NVIDIA SN2100 cluster switches. Refer to link:configure-overview-sn2100-cluster.html[Overview of installation and configuration for NVIDIA SN2100 switches].
...
For details of supported ports and their configurations, refer to the link:https://hwu.netapp.com/[Hardware Universe^] .
...
NOTE: If you are changing the port speed of the e0a and e1a cluster ports on AFF A800 or AFF C800 systems, you might observe malformed packets being received after the speed conversion. Refer to link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1570339[Bug 1570339^] and the Knowledge Base article link:https://kb.netapp.com/onprem/ontap/hardware/CRC_errors_on_T6_ports_after_converting_from_40GbE_to_100GbE[CRC errors on T6 ports after converting from 40GbE to 100GbE^] for guidance.
...
. On switch sw2, shut down and restart all cluster ports to trigger an auto-revert of all cluster LIFs that are not on their home ports.
+
...
[start=3]
. [[step3]]Verify that the cluster LIFs have reverted to their home ports (this might take a minute):
+
`network interface show -vserver Cluster`
+
