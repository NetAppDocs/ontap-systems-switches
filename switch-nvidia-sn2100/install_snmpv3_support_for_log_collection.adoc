...
* For MD5/SHA authentication, run:
`net add snmp-server username [SNMPv3_USER] [auth-md5/auth-sha] [AUTH-PASSWORD]`
* For MD5/SHA authentication with AES/DES encryption, run:
...
The following procedure sets up the SNMPv3 user on the switch to use authentication and encryption.

.Steps
. Setup the SNMP user on the switch:
+
[subs=+quotes]
----
