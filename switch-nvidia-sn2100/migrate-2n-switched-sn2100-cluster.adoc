...
summary: "Migrate an existing two-node switchless cluster to a switched cluster environment using NVIDIA SN2100 cluster switches."
---
...
[.lead]
Migrate an existing two-node switchless cluster to a switched cluster environment using NVIDIA SN2100 cluster switches. This nondisruptive migration enables you to scale the cluster beyond two nodes. 
...
+
IMPORTANT: Refer to link:cabling-considerations-sn2100-cluster.html[Review cabling and configuration considerations] for caveats and further details. The link:https://hwu.netapp.com/SWITCH/INDEX[Hardware Universe - Switches^] also contains more information about cabling.
...
[start=8]
. [[step8]]Change the privilege level back to admin: 
+
`set -privilege admin`
