...
CAUTION: The default password for the cumulus user account is *cumulus*. The first time you log into Cumulus Linux, you must change this default password. Make sure to update any automation scripts before installing a new image. Cumulus Linux provides command line options to change the default password automatically during the installation process. 
...
[subs=+quotes]
----
cumulus@cumulus_sw1:mgmt:~$ *nv set interface eth0 ip address 10.231.80.206*
cumulus@cumulus_sw1:mgmt:~$ *nv set interface eth0 ip gateway 10.231.80.1*
cumulus@cumulus_sw1:mgmt:~$ *nv config apply*
applied [rev_id: 2]
cumulus@cumulus_sw1:mgmt:~$ *ip route show vrf mgmt*
...
[subs=+quotes]
----
cumulus@cumulus_sw1:mgmt:~$ *sudo adduser --ingroup netedit admin*
[sudo] password for cumulus:
Adding user `admin' ...
...
cumulus@cumulus_cs1:mgmt:~$ *sudo adduser admin sudo*
[sudo] password for cumulus:
Adding user `admin' to group `sudo' ...
Adding user admin to group sudo
Done.
cumulus@cumulus_sw1:mgmt:~$ *exit*
logout
...
[subs=+quotes]
----
cumulus@cumulus_sw1:mgmt:~$ *sudo adduser admin nvshow*
     [sudo] password for cumulus:
...
[subs=+quotes]
----
admin@sw1:mgmt:~$ *nv set system hostname cumulus_cs1*
admin@sw1:mgmt:~$ *nv config apply*
Warning: The following files have been changed since the last save, and they WILL be overwritten.
- /etc/nsswitch.conf
- /etc/synced/synced.conf
.
...
[subs=+quotes]
----
admin@sw1:mgmt:~$ *nv set interface eth0 ip address 10.231.80.206*
admin@sw1:mgmt:~$ *nv set interface eth0 ip gateway 10.231.80.1*
admin@sw1:mgmt:~$ *nv config apply*
applied [rev_id: 2]
admin@sw1:mgmt:~$ *ip route show vrf mgmt*
default via 10.231.80.1 dev eth0 proto kernel
unreachable default metric 4278198272
10.231.80.0/22 dev eth0 proto kernel scope link src 10.231.80.206
127.0.0.0/8 dev mgmt proto kernel scope link src 127.0.0.1
...
[subs=+quotes]
----
admin@sw1:mgmt:~$ *sudo adduser --ingroup netedit admin*
[sudo] password for cumulus:
Adding user `admin' ...
Adding new user `admin' (1001) with group `netedit' ...
Creating home directory `/home/admin' ...
...
admin@sw1:mgmt:~$ *sudo adduser admin sudo*
[sudo] password for cumulus:
Adding user `admin' to group `sudo' ...
Adding user admin to group sudo
Done.
...
[subs=+quotes]
----
admin@sw1:mgmt:~$ *sudo adduser admin nvshow*
     [sudo] password for cumulus:
     Adding user `admin' to group `nvshow' ...
     Adding user admin to group nvshow
     Done.
