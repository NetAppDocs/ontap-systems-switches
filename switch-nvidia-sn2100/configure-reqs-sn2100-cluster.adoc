...
summary: "Review the configuration requirements for NVIDIA SN2100 cluster switches before installation or upgrade."
---
= Review configuration requirements for NVIDIA SN2100 cluster switches
:icons: font
:imagesdir: ../media/

[.lead]
Review the configuration requirements for NVIDIA SN2100 cluster switches before installation or upgrade. 
...
For cabling guidelines, refer to link:cabling-considerations-sn2100-cluster.html[Review cabling and configuration considerations].
...
** Specific CL versions are qualified and supported by NetApp. For current compatibility information, refer to link:https://mysupport.netapp.com/site/info/nvidia-cluster-switch[NVIDIA Ethernet Switches information^] or the link:https://hwu.netapp.com[Hardware Universe^].

** In order to download the SN2100 Cumulus software from NVIDIA, you must have login credentials to access NVIDIA's Enterprise Support Portal. Refer to the Knowledge Base article link:https://kb.netapp.com/onprem/Switches/Nvidia/How_To_Register_With_NVIDIA_For_Enterprise_Support_Portal_Access[How to register with NVIDIA for Enterprise Support Portal Access^]. 
