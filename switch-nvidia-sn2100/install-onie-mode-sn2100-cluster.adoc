...
summary: "Install Cumulus Linux on NVIDIA SN2100 cluster switches when the switch is running in ONIE mode."
---
= Install Cumulus Linux in ONIE mode on NVIDIA SN2100 cluster switches
:icons: font
:imagesdir: ../media/

[.lead]
Install Cumulus Linux (CL) on NVIDIA SN2100 cluster switches when the switch is running in ONIE mode.  

NOTE: Cumulus Linux (CL) OS can be installed either when the switch is running ONIE or Cumulus Linux (refer to link:install-cumulus-mode-sn2100-cluster.html[Install in Cumulus mode]).
...
.What's next?
After you've installed Cumulus Linux in ONIE mode, you can link:install-rcf-sn2100-cluster.html[install the Reference Configuration File (RCF) script].
