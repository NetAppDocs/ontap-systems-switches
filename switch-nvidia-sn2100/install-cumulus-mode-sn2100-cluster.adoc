...
------------------- -------------------- ---------------------
hostname            cumulus              cumulus
build               Cumulus Linux 5.4    system build version
uptime              6 days, 8:37:36      system uptime
timezone            Etc/UTC              system time zone
----
...
. Install Cumulus Linux 5.11.1:
+
[subs=+quotes]
----
cumulus@sw1:mgmt:~$ *sudo onie-install -a -i http://_<web-server>/<path>_/cumulus-linux-5.11.1-mlx-amd64.bin*
----
The installer starts the download. Type *y* when prompted.
...
* Cumulus-Linux GNU/Linux
. Repeat steps 1 to 4 to log in.
. Verify that the Cumulus Linux version is 5.11.1: 
+
`nv show system`
+
[subs=+quotes]
