...
* NVIDIA SN2100: ports swp15-16 (40/100GbE)

The https://hwu.netapp.com/Home/Index[_NetApp Hardware Universe_^] contains information about ONTAP compatibility, supported Cumulus Linux (CL) OS, and cabling to SN2100 cluster switches.
...
* The SN2100 switches are pre-loaded with the supported versions of the Reference Configuration File (RCF) and Cumulus Linux OS with ISL cables connected on ports swp15 and swp16.
...
. Verify that the appropriate port licenses, RCF, and CL OS image are installed on the new SN2100 switches as necessary for your requirements, and make any essential site customizations, such as users and passwords, network addresses, and so on.
...
+
You must attach appropriate approved cabling connecting port swp15 to swp16 on the new SN2100 switch cs2, to ports 13 to 16 on the existing CN1610 switch CL1.
