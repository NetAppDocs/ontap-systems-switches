...
.. Delete all NOD files corresponding to your connected switch models. 
+
If unsure, remove all NOD files for the supported models listed above, then download and install the latest NOD files for those same models.
+
`rm /etc/cshm_nod/nod_sign/<filename>`
.. Confirm that the deleted NOD files are no longer in the directory:
+
`ls /etc/cshm_nod/nod_sign`
