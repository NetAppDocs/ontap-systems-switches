...
* Clear the `/mroot/etc/cshm_nod/nod_sign/` directory on all nodes as follows: 
.. Enter the nodeshell:
+
...
+
`priv set advanced`
.. List the configuration files in the `/etc/cshm_nod/nod_sign` directory:
+
`ls /etc/cshm_nod/nod_sign`
+
If the directory exists and contains configuration files, it lists the file names.
.. Delete all of the NOD files with names corresponding to the switch models connected:
+
`rm /etc/cshm_nod/nod_sign/<filename>`
.. Confirm that the directory no longer contains any NOD files:
+
`ls /etc/cshm_nod/nod_sign`
