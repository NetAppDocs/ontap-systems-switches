...
[cols="1,2"]
|===
| Cluster | On ports configured for 4x10GbE breakout, one port is configured for 4x25GbE breakout and the other ports are configured for 40/100GbE. Supports shared cluster/HA traffic on ports for nodes that use shared cluster/HA ports. See the platform table in the Knowledge Base article https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_AFF_ASA_and_FAS_platforms_use_shared_Cluster_and_HA_Ethernet_ports[What AFF, ASA, and FAS platforms use shared Cluster and HA Ethernet ports?^]. All ports can be used as dedicated cluster ports too.
a|
Storage
a|
