...
[start=1]
.[[step1]]Display the available interfaces on the SN2100 switch:
...
[start=2]
.[[step2]]Copy the RCF python script to the switch.
...
[start=3]
.[[step3]]Apply the RCF python script *MSN2100-RCF-v1.x-Cluster*.
...
[start=4]
.[[step4]]Verify the configuration after the reboot:
...
[start=5]
.[[step5]]Verify information for the transceiver in the interface:
...
[start=6]
.[[step6]]Verify that the nodes each have a connection to each switch:
