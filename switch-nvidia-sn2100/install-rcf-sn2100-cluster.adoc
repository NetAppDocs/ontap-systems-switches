...
summary: "Install or upgrade the reference configuration file (RCF) script on NVIDIA SN2100 cluster switches."
---
= Install or upgrade the RCF script on NVIDIA SN2100 cluster switches
:icons: font
:imagesdir: ../media/

[.lead]
Install or upgrade the reference configuration file (RCF) script on NVIDIA SN2100 cluster switches to configure port assignments and network settings.

.Before you begin
Before installing or upgrading the RCF script, make sure that the following are available on the switch:

* Cumulus Linux is installed. Refer to the link:https://hwu.netapp.com/Switch/Index[Hardware Universe^] for supported versions.
...
The following reference configuration categories are available:

[cols="1,2"]
|===
| Cluster | On ports configured for 4x10GbE breakout, one port is configured for 4x25GbE breakout and the other ports are configured for 40/100GbE. Supports shared cluster/HA traffic on ports for nodes that use shared cluster/HA ports. Refer to the platform table in the Knowledge Base article link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_AFF_ASA_and_FAS_platforms_use_shared_Cluster_and_HA_Ethernet_ports[What AFF, ASA, and FAS platforms use shared Cluster and HA Ethernet ports?^]. All ports can be used as dedicated cluster ports too.
a|
Storage
a|
All ports configured for 100GbE NVMe storage connections. 
|===
...
----
+
NOTE: While `scp` is used in the example, you can use your preferred method of file transfer, for example SFTP, HTTPS, or FTP.
...
. Reapply any previous customizations to the switch configuration. Refer to link:cabling-considerations-sn2100-cluster.html[Review cabling and configuration guidelines] for details of any further changes required.
...
----
+
NOTE: Although `scp` is used in the example, you can use your preferred method of file transfer, for example SFTP, HTTPS, or FTP.
...
.What's next?
After you've installed the RCF, you can link:setup-install-cshm-file.html[install the CSHM file].
...
// Updates for AFFFASDOC-250, 2025-AUG-25
// Updates for AFFFASDOC-307, 2025-JUN-25
// Updates for AFFFASDOC-275, 2025-AUG-05
