...
The RCF script completes the steps listed in the example above.
+
NOTE: In step 3 *Updating the MOTD file* above, the command `cat /etc/issue` is run. This allows you to verify the RCF filename, RCF version, ports to use and other important information in the RCF banner. For example:
----
