...
cumulus@cumulus:mgmt:~$ *net add interface eth0 ip address _<ip_address>_*
cumulus@cumulus:mgmt:~$ *net add interface eth0 ip gateway _<ip_gateway>_*
...
[subs=+quotes]
----
cumulus@sw1:mgmt:~$ *sudo onie-install -a -i _<file_location>_/cumulus-linux-4.4.2-mlx-amd64.bin*
----
