...
summary: "Install the reference configuration file (RCF) on a Cisco Nexus 9336C-FX2 shared switch after initial setup to configure port assignments for combined cluster and storage operation."
---
...
[.lead]
Install the reference configuration file (RCF) on a Cisco Nexus 9336C-FX2 shared switch after initial setup. The RCF configures port assignments for combined cluster and storage operation. 
...
* *ClusterStorageRCF* - Supports a partitioned cluster plus two storage zones on the switches (Cluster-Storage RCF 1._xx_). 
...
. Repeat steps 1 through 8 on switch cs2.
