...
keywords:
summary: "Migrate from a switchless configuration with switch-attached storage to Cisco Nexus 9336C-FX2 shared switches by reusing the existing storage switches."
---
= Migrate from a switchless configuration with switch-attached storage to Cisco Nexus 9336C-FX2 shared switches
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Migrate from a switchless configuration with switch-attached storage to Cisco Nexus 9336C-FX2 shared switches by reusing the existing storage switches.
...
[start=7]
. [[step7]]Replace the storage RCF on shared switch A with the shared RCF file. Refer to link:install-nxos-rcf-9336c-shared.html[Install the RCF on a Cisco Nexus 9336C-FX2 shared switch] for further details.
...
[start=13]
. [[step13]]Replace the storage RCF file on shared switch B with the shared RCF file. Refer to link:install-nxos-rcf-9336c-shared.html[Install the RCF on a Cisco Nexus 9336C-FX2 shared switch] for further details.
. Verify the storage attached to HA pair 1, storage switch B is healthy:
+
`system health alert show -instance`
+
...
[start=16]
. [[step16]]Convert HA pair 1 from a switchless cluster to a switched cluster. Use the cluster port assignments defined by the shared RCF. Refer to link:prepare-nxos-rcf-9336c-shared.html[Install NX-OS software and reference configuration files (RCFs)]​ for further details.
