...
keywords:
summary: "Migrate from a switched cluster with switch-attached storage to Cisco Nexus 9336C-FX2 shared switches by reusing the existing storage switches."
---
= Migrate from a switched cluster with switch-attached storage to Cisco Nexus 9336C-FX2 shared switches
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Migrate from a switched cluster with switch-attached storage to Cisco Nexus 9336C-FX2 shared switches by reusing the existing storage switches as shared switches.
...
[start=6]
. [[step6]]Replace the storage RCF on shared switch A with the shared RCF file. Refer to link:install-nxos-rcf-9336c-shared.html[Install the RCF on a Cisco Nexus 9336C-FX2 shared switch] for further details.
. Verify the storage attached to HA pair 1, storage switch A is healthy:
+
...
[start=12]
. [[step12]]Replace the storage RCF file on shared switch B with the shared RCF file. Refer to link:install-nxos-rcf-9336c-shared.html[Install the RCF on a Cisco Nexus 9336C-FX2 shared switch] for further details.
