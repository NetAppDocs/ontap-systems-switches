...
. Verify that the expected nodes are connected to the correct devices.
+
`show cdp neighbors`
+
.Show example 
...
. Verify that the cluster nodes are in their correct cluster VLANs using the commands:
+
`show vlan brief`
+
