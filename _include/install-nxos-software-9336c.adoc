...
[NOTE]
=======
The EPLD is not updated when you upgrade to NX-OS 10.5(3) or later. 
After your upgrade completes and the switch reboots, you must run the same upgrade again.

* The second upgrade pass verifies, but skips the NX-OS upgrade; only the EPLD is updated.
...
NOTE: The following step is only required if you are running an earlier version of the NX-OS. You must perform the EPLD image upgrade as a separate step after the NX-OS upgrade and reboot.
