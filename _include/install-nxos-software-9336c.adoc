...
. Upgrade the version of NX-OS and EPLD using the applicable procedure.
...
[start=d]
.. [[stepd]] Verify the new version of NX-OS software after the switch has rebooted:
+
`show version`
+
...
--
[start=a]
.. [[stepa]] Copy NX-OS and EPLD image to the first switch.
+
`copy <protocol>: bootflash: vrf management`
+
...
....
cs2# *install all nxos bootflash:nxos64-s1.10.6.3.F.bin*
