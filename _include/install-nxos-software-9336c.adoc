...
[start=6]
. [[step6]] Copy the NX-OS software and EPLD images to the Nexus 9336C-FX2 switch.
+
...
--
[start=6]
. [[step6]]Copy the NX-OS software and EPLD image to the Nexus 9336C-FX2 switch.
+
`copy <protocol>: bootflash: vrf management`
+
...
[start=9]
. [[step9]] Verify the new version of NX-OS software after the switch has rebooted:
+
`show version`
+
...
. Skip to step 12 to verify the health of cluster ports and switch health from the cluster after the upgrade.
--

.Upgrade from 10.5(3)F to a later version
--
[start=6]
. [[step6]]Copy NX-OS and EPLD image to the first switch.
+
`copy <protocol>: bootflash: vrf management`
+
...
----
cs2# *copy sftp: bootflash: vrf management*
Enter source filename: */code/nxos-s1.10.6.3.F.bin*
Enter hostname for the sftp server: *172.19.2.1*
Enter username: *root*
...
sftp> *progress*
Progress meter enabled
sftp> get /code/nxos-s1.10.6.3.F.bin /bootflash/nxos-s1.10.6.3.F.bin
/code/nxos-s1.10.6.3.F.bin  100% 1261MB   9.3MB/s   02:15
sftp> *exit*
Copy complete, now saving to disk (please wait)...
Copy complete.
----
...
Software
  ...
  NXOS image file is: bootflash:///nxos-s1.10.5.3.F.bin
...
[subs=+quotes]
----
cs2# *install all nxos bootflash:nxos-s1.10.6.3.F.bin*

Installer will perform compatibility check first. Please wait.
Installer is forced disruptive

Verifying image bootflash:/nxos-s1.10.6.3.F.bin for boot variable "nxos".
[####################] 100% -- SUCCESS

Verifying image type.
[####################] 100% -- SUCCESS

Preparing "nxos" version info using image bootflash:/nxos-s1.10.6.3.F.bin.
[####################] 100% -- SUCCESS

Preparing "bios" version info using image bootflash:/nxos-s1.10.6.3.F.bin.
[####################] 100% -- SUCCESS
...
Software
  ...
  BIOS compile time:  11/25/2024
  NXOS image file is: bootflash:////nxos-s1.10.6.3.F.bin
...
[start=12]
. [[step12]]Verify the health of cluster ports on the cluster.
