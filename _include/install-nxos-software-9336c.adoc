...
.Upgrade to 10.3.5(m) and earlier
--
...
Enter source filename: /code/nxos.10.3.5.M.bin
...
sftp> get   /code/nxos.10.3.5.M.bin  /bootflash/nxos.10.3.5.M.bin
/code/nxos.10.3.5.M.bin  100% 1261MB   9.3MB/s   02:15
sftp> exit
...
cs2# *copy sftp: bootflash: vrf management*
Enter source filename: /code/n9000-epld.10.3.5.M.img
Enter hostname for the sftp server: 172.19.2.1
Enter username: user1
...
sftp> progress
Progress meter enabled
sftp> get   /code/n9000-epld.10.3.5.M.img  /bootflash/n9000-epld.10.3.5.M.img
/code/n9000-epld.10.3.5.M.img  100%  161MB   9.5MB/s   00:16
sftp> exit
Copy complete, now saving to disk (please wait)...
Copy complete.
----
...
[subs=+quotes]
----
cs2# *install all nxos bootflash:nxos.10.3.5.M.bin*

Installer will perform compatibility check first. Please wait.
Installer is forced disruptive

Verifying image bootflash:/nxos.10.3.5.M.bin for boot variable "nxos".
[] 100% -- SUCCESS

Verifying image type.
[] 100% -- SUCCESS

Preparing "nxos" version info using image bootflash:/nxos.10.3.5.M.bin.
[] 100% -- SUCCESS

Preparing "bios" version info using image bootflash:/nxos.10.3.5.M.bin.
[] 100% -- SUCCESS
...
Software
  BIOS: version 05.33
  NXOS: version 10.3.5(M)  BIOS compile time:  09/08/2018
  NXOS image file is: bootflash:///nxos.10.3.5.M.bin
...
cs2# *install epld bootflash:n9000-epld.10.3.5.M.img module all*
Compatibility check:
...
[NOTE]
=======
The EPLD image is not automatically updated when you upgrade to NX-OS 10.5(3) and later. After your upgrade completes and the switch reboots, you must run the same upgrade `install all` command again.

* The second upgrade pass verifies, but skips the NX-OS upgrade; only the EPLD image is updated.
* When you have already upgraded to NX-OS 10.5(3) and later, both images are updated at the same time.
=======

Installing the image file causes it to be loaded every time the switch is rebooted.


The following step shows an upgrade from NX-OS 10.3.5(m) to 10.5.3(f). The first pass performs the NX-OS upgrade and the second pass performs the EPLD image upgrade.

[subs=+quotes]
----
cs2# *install all nxos bootflash:nxos64-cs.10.5.3.F.bin*   <---- 1st pass updates NX-OS only

Installer will perform compatibility check first. Please wait.
Installer is forced disruptive

Verifying image bootflash:/nxos64-cs.10.5.3.F.bin for boot variable "nxos".
[####################] 100% -- SUCCESS

Verifying image type.
[####################] 100% -- SUCCESS

Preparing "nxos" version info using image bootflash:/nxos64-cs.10.5.3.F.bin.
[####################] 100% -- SUCCESS

Preparing "bios" version info using image bootflash:/nxos64-cs.10.5.3.F.bin.
[####################] 100% -- SUCCESS
...
cs2# *install all nxos bootflash:nxos64-cs.10.5.3.F.bin*   <---- 2nd pass updates EPLD image only
 
Installer will perform compatibility check first. Please wait. 
Installer is forced disruptive
 
...
[####################] 100% -- SUCCESS
 
Verifying EPLD/FPGA image //bootflash/nxos64-cs.10.5.3.F.bin.
[####################] 100% -- SUCCESS
 
Verifying image type.
[####################] 100% -- SUCCESS
