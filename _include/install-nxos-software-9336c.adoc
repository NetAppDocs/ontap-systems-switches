...
This is the current content in here.


--


.from a release earlier than 10.5(3)f to 10.5(3)f
--


This is the update from Tim McC in here


--

.to 10.5(3)f and later
--


This is new content for latest NX-OS version
...
. Upgrade the EPLD image and reboot the switch. If you are using NX-OS 10.5(3)F or later, you do not need to perform a separate EPLD image upgrade because the EPLD image is included in the NX-OS image. Skip this step and continue with verifying cluster port health.
...
*HERE IS 2ND TAB CONTENT*
. Copy the NX-OS software and EPLD images to the Nexus 9336C-FX2 switch.
+

NOTE: If you upgrade from NX-OS 10.5(3) or later, both images update at the same time. You do not need to install an EPLD image by itself. When you copy the NX-OS image, you also copy the EPLD image.
...
----
cs2# *copy sftp: bootflash: vrf management*
Enter source filename: */code/nxos64-cs.10.5.4.M.bin*
Enter hostname for the sftp server: *172.19.2.1*
Enter username: *user1*
...
Software
  ...
  BIOS compile time:  05/29/2025
  NXOS image file is: bootflash:///nxos64-cs.10.5.4.M.bin
  NXOS compile time:  4/28/2025 21:00:00 [04/29/2025 02:28:31]
...
. Install the NX-OS image.
When you install the image file, the switch loads it every time it reboots.
+

[NOTE]
=======
The EPLD image is not automatically updated when you upgrade to NX-OS 10.5(3) or later. After your upgrade completes and the switch reboots, you must run the same upgrade `install all` command again.

* The second upgrade pass verifies, but skips the NX-OS upgrade; only the EPLD image is updated.
* When you have already upgraded to NX-OS 10.5(3) or later, both images are updated at the same time.
=======
+

The following step shows an upgrade from NX-OS 10.3(4a) to 10.5(4). The first pass performs the NX-OS upgrade and the second pass performs the EPLD image upgrade.
...
[subs=+quotes]
----
cs2# *install all nxos bootflash:nxos64-cs.10.5.4.M.bin*   <---- 1st pass updates NX-OS only
...
cs2# *install all nxos bootflash:nxos64-cs.10.5.4.M.bin*   <---- 2nd pass updates EPLD image only
 
Installer will perform compatibility check first. Please wait. 
Installer is forced disruptive
 
...
[####################] 100% -- SUCCESS
 
Verifying EPLD/FPGA image //bootflash/nxos64-cs.10.5.4.M.bin.
[####################] 100% -- SUCCESS
 
Verifying image type.
[####################] 100% -- SUCCESS
...
Software
  BIOS: version 05.53
  NXOS: version 10.5(4)
  BIOS compile time:  09/08/2025
  NXOS image file is: bootflash:///nxos64-cs.10.5.4.M.bin
  NXOS compile time:  11/4/2025 21:00:00 [11/05/2025 06:11:06]
...
Last reset at 277524 usecs after Mon Nov  2 22:45:12 2025
  Reason: Reset due to upgrade
  System version: 10.5(4)
  Service:
...
NOTE: If you are using NX-OS 10.5(3)F or later, you do not need to perform a separate EPLD image upgrade because the EPLD image is included in the NX-OS image. Skip this step and continue with verifying cluster port health.
...
[start=11]
. [[step11]] After the switch reboot, log in again and verify that the new version of EPLD loaded successfully.
+
