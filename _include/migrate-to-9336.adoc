...
. On switch cs2-new, shut down and restart all cluster ports to trigger an auto-revert of all cluster LIFs that are not on their home ports.
+
...
cs2-new> *enable*
cs2-new# *configure*
cs2-new(config)# *interface eth1/1-1/2*
cs2-new(config-if-range)# *shutdown*

(Wait for 5-10 seconds before re-enabling the ports)

cs2-new(config-if-range)# *no shutdown*
...
.What's next?
After you've migrated the switches, you can link:../switch-cshm/config-overview.html[configure switch health monitoring].
