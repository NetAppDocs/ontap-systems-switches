...
---
= Learn about ONTAP system switches

:icons: font
:imagesdir: ../media/

[.lead]
ONTAP offers AFX systems, cluster, storage, and shared switches that deliver internal communications with the ability to non-disruptively move data and network interfaces across the cluster.
...
CAUTION: If you are upgrading your switches, NetApp recommends that you complete any ONTAP software upgrade first, then perform the software upgrade for your switches.

== AFX systems switches
AFX systems switches allow you to create fast, energy-efficient, and space-saving network switches. Their small size and high port density make them ideal for handling large networks. The ports support different speeds and can be split into multiple connections.  NetApp-supported AFX systems switches include:
...
For a list of switches that are no longer available for purchase, but are still supported, refer to link:../eoa/index.html[End-of-availability switches].
