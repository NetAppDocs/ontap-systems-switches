...
+
There is one RCF 10.0 file and it supports shared switches, `Cluster-HA-Storage AFX RCF_10.0`.
+
...
+
There are different RCF 10.1 files for different use cases. Depending on how you plan to use the switches in your AFX system configuration (shared, storage only, or cluster and HA only), you choose the applicable RCF file.
+
For example, if you are using your switches as shared switches, you use the RCF file `RCF-v10.1-NX9332D-GX2B-CL1-ST1-100GBR15-400G15.txt`. The following table shows the preset groups of the switch ports, their port speeds, and usage for this RCF file.
+
[options="header" cols="1,2"]
|===
| Switch port groups | Port speeds and usage
