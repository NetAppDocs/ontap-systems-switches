---
permalink: switch-cisco-9332d-gx2b/maintain-replace-switch-9332d.html
sidebar: sidebar
...
summary: Replace a Cisco Nexus 9332D-GX2B switch when it is defective. The replacement procedure is nondisruptive (NDU). It involves reviewing the procedure requirements, preparing the storage system and hardware, replacing the switch, configuring the cables and ports on the replacement switch, and then verifying the configuration.
---
= Replace a switch (being updated)- Cisco Nexus 9332D-GX2B
:icons: font
:imagesdir: ../media/

[.lead]
Replace a Cisco Nexus 9332D-GX2B switch when it is defective. The replacement procedure is nondisruptive (NDU). It involves reviewing the procedure requirements, preparing the storage system and hardware, replacing the switch, configuring the cables and ports on the replacement switch, and then verifying the configuration.

.About this task
* NetApp recommends that you enable console logging on your storage system to capture the output of the console during this procedure. This can be helpful for troubleshooting if you encounter any issues.
+
For instructions on how to enable session logging, review the "Logging Session Output" section in this Knowledge Base article https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/How_to_configure_PuTTY_for_optimal_connectivity_to_ONTAP_systems[How to configure PuTTY for optimal connectivity to ONTAP systems^].
...
* The examples shown in this procedure are for AFX A1K systems with switches running RCF 10.0. If you have an AFX A2K system, the output will vary.
 ...
 ** Management network connectivity and console access to the replacement switch is in place.
 ** The node connections are ports 1/1 through 1/30.
 ** All Inter-Switch Link (ISL) ports is disabled on ports 1/31 and 1/32.
 ** The appropriate reference configuration file (RCF) and NX-OS operating system image switch is loaded onto the switch.
+
link:configure-prepare-install-nxos-rcf-9332d.html[Prepare to install the NX-OS and RCF] on the switch.
+
link:configure-install-upgrade-nxos-9332d.html[Install or upgrade the NX-OS] on the switch.
+
...
Lisa: if we're doing all the switch config steps when switch is on the cart - before swapping it out in the rack, then need to include SSH and CSHM too. I feel like having CSHM in the What's next part of this topic is too late...unless that requires connectivity to the system...same for SSH? But SSH is ignored in this procedure...unless I haven't seen it yet.

link:configure-verify-ssh-9332d.html[Verify SSH for log collection] so that you can use the Ethernet switch health monitor (CSHM) log collection feature.
...
** Initial customization of the switch is complete, as detailed in link:configure-complete-initial-setup-9332d.html[Complete initial switch setup].
+
Any previous site customizations, such as STP, SNMP, and SSH, are copied to the new switch.
+
Lisa: Use the configuration wizard to complete the initial switch setup. 
//assume this popped up when the new switch got powered on.
+
Lisa: If needed, you can reference the configuration wizard questions and recommended responses in the link:configure-complete-initial-setup-9332d.html[Complete initial switch setup] procedure.

* If you are replacing the physical switch chassis, complete the unrack/rack tasks in install hardware before reconnecting data cables in this procedure.
...
Prepare the storage system so that it is safe to replace the switch without causing disruption to client workloads.
...
. If you have not already done so, install the appropriate RCF and image on the switch, newcs2, and make any necessary site preparations.
+
If necessary, verify, download, and install the appropriate versions of the RCF and NX-OS software for the new switch. However, if you have verified that the new switch is correctly set up and does not need updates to the RCF and NX-OS software, continue to step 3.
+
Otherwise:

  .. Go to link:https://mysupport.netapp.com/site/products/all/details/cisco-cluster-storage-switch/downloads-tab[Cisco switches^] on the NetApp Support Site.
 .. Select link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch[Switch compatibility matrix^] to determine your required RCF version.
 .. From the *Downloads* dropdown list, select the applicable RCF version and select *Go*. 
 .. Accept the caution/must read and the end user license agreement, and select *Accept & Continue*.
 .. Select the RCF and/or accompanying release notes to download. 
 .. Follow the steps at link:https://www.cisco.com/c/en/us/support/switches/nexus-9332d-gx2b-switch/model.html[Cisco Nexus 9332D-GX2B Switch^] to download the correct NX-OS files for the version of ONTAP software you are installing.
+
NOTE: NX-OS 10.4.2 is the only currently supported version for Cisco Nexus 9332D-GX2B switches in an ONTAP cluster.
+
...
== Step 3: Configure cables and ports
...
. Bring up all of the ports, Eth1/1 to Eth1/30, on the newcs2 switch.
+
.Show example 
[%collapsible]
====
...
Verify the health of all the ports on the storage system (cluster, HA, storage, and storage shelf) and re-enable automatic case creation as needed.

. Verify that cluster ports are up and healthy across all nodes in the cluster by using the following commands in the order shown:
+
[source,cli]
----
network port show ipspace Cluster
...
. Verify the health status and connection status of all the storage shelf ports.
...
After you've replaced the switch, you link:../switch-cshm/config-overview.html[configure switch health monitoring]. Really? Does new switch have to be cabled to system before you can configure CSHM? If yes, then leave this as is.
