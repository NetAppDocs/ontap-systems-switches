...
* HEY, June 10: this bullet (I addded the CMD) was in here originally. But same CMD is in Install RCF, so bullet should be added there too?
+
You must be connected to the switches using the serial console or have preserved basic configuration information. This is because the RCF upgrade procedure requires you to erase the switch settings (using the `write erase` command) and perform basic configuration. 
//You must be connected to the switches using the serial console or have preserved basic configuration information. This is because the RCF upgrade procedure requires you to erase the switch settings (using the `write erase` command) and perform basic configuration.
...
Using the ONTAP CLI, prepare for the RCF upgrade by suppressing automatic case creation as needed, checking administrative and operational status on the ports, and disabling auto-revert on the cluster LIFs.
...
You use the switch CLI and the ONTAP CLI to install the RCF on the the first switch (cs1) and then repeat these steps for the second switch (cs2).

. Using the switch CLI, on switch cs1, shut down the ports connected to all the ports of the nodes.
+
...
====
+
. If you have not already done so, using the switch CLI, save a copy of the current switch configuration by copying the output of the following command to a text file:
+
`show running-config`
+
...
. Using the switch CLI, verify that the `write_erase.cfg` file is populated as expected:
+
`*show file bootflash:write_erase.cfg*`

. Using the switch CLI, issue the `write erase` command to erase the current saved configuration:
+
`cs1# *write erase*`
+
...
. Using the switch CLI, copy the RCF to the bootflash of switch cs1 using one of the following transfer protocols: FTP, TFTP, SFTP, or SCP. 
+
...
. Using the switch CLI, apply the RCF previously downloaded to the bootflash.
+
For more information on Cisco commands, refer to the appropriate guide in the https://www.cisco.com/c/en/us/support/switches/nexus-9336c-fx2-switch/model.html#CommandReferences[Cisco Nexus 9000 Series NX-OS Command Reference^] guides.
...
. Using the switch CLI, verify that the RCF file is the correct newer version: 
+
`show running-config`
+
...
+
If your switches are running RCF 10.1 or later, you can link:configure-install-upgrade-rcf-overview-9332d.html#change-port-speed-configuration[change the port speed configuration] if needed.

. Using the switch CLI, after you verify the RCF versions, custom additions, and switch settings are correct, copy the `running-config` file to the `startup-config` file.
+
For more information on Cisco commands, refer to the appropriate guide in the https://www.cisco.com/c/en/us/support/switches/nexus-9336c-fx2-switch/model.html#CommandReferences[Cisco Nexus 9000 Series NX-OS Command Reference^] guides.
+
...
. Using the ONTAP CLI, enable auto-revert on the cluster LIFs.
+
[source,cli]
----
...
. Using the switch CLI, verify that the switch ports connected to the cluster ports are *up*.
+
[source,cli]
----
...
. Using the switch CLI, verify that the expected nodes are still connected:
+
[source,cli]
----
show cdp neighbors
...
. Using the ONTAP CLI, verify the connectivity of the remote cluster interfaces: 
+
...
After you've upgraded the RCF on your switches, you link:configure-verify-ssh-9332d.html[verify SSH for log collection].
