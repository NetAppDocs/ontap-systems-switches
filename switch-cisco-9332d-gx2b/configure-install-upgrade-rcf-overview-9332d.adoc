...
. Hey, for new step below, seems since no ports are called out in these 2 cmds, that these are part of step above...seems like it based on format of hard-code cmds in that topic.
...
+
For example, to hard-code 100GbE on breakout ports 15-19, do the following. You must enter these commands for both switches.
+
`speed 100000`
+
`no negotiate auto`
...
After you've reviewed the install or upgrade RCF overview, you link:configure-install-rcf-9332d.html[install the RCF] or link:configure-upgrade-rcf-9332d.html[upgrade the RCF] as needed.
