...
** Initial customization of the switch is complete, as detailed in link:configure-complete-initial-setup-9332d.html[Configure the 9332D-GX2B cluster switch].
+
Any previous site customizations, such as STP, SNMP, and SSH, are copied to the new switch.

//Doesn't make sense here. we want users to swap out hw after new switch is configured - to minimize downtime for the faulty switch that could still be operating. And first part of bullet is redundant...this is a topic about replacing the physical switch.
...
. If you have not already done so, install the appropriate NX-OS and RCF images on the switch, newcs2, and make any necessary site preparations.
+
If necessary, verify, download, and install the appropriate versions of the NX-OS and RCF for the new switch. However, if you have verified that the new switch is correctly set up and does not need updates to the RCF and NX-OS, continue to step 3.
+
Otherwise:
...
// NOT YET DONE: Refactor content project phase 1 with 2K/RCF 10.1 content, GH#423 2026-JUN