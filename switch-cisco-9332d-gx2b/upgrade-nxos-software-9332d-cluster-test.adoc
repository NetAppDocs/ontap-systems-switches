---
permalink: switch-cisco-9332d-gx2b/upgrade-nxos-software-9332d-cluster-test.html
sidebar: sidebar
...
---
= Upgrade the NX-OS software - Cisco Nexus 9332D-GX2B
:icons: font
:imagesdir: ../media/

[.lead]
You upgrade the NX-OS on the Cisco 9332D-GX2B switches as part of a new AFX system installation, a switch replacement, or a reset to factory default settings. 

Hey, if only NX-OS 10.4.2 is supported, why does there need to be an upgrade procedure if you can only run ONE version???????
...
//Lisa these below ONLY apply to the upgrade NX-OS topic. Need an include for these: cisco-nxos-upgrade.adoc or something like that.To be reused for all Cisco NX-OS upgrade topics...I think.

//include::../_include/cisco-nxos-upgrade-suggested-documents.adoc[]
...
== Step 2: Prepare to upgrade the NX-OS

Prepare to upgrade the NX-OS. 
...
== Step 3: Upgrade the NX-OS

You upgrade the NX-OS on one of the switches and then you repeat these steps to upgrade NX-OS on the second switch.
...
. Repeat these steps to install the NX-OS software and EPLD images on switch cs1.
...
== Step 4: Complete the NX-OS upgrade

Complete the NX-OS upgrade on both switches by verifying that the cluster LIFs have reverted to their home ports and then enable auto-revert on the cluster LIFs.
...
After you've installed NX-OS on your switches, you link:install-upgrade-rcf-overview-cluster.html[install or upgrade the Reference Configuration File (RCF)].
