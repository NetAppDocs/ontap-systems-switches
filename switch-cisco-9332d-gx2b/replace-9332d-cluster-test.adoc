...
. Lisa does it make sense to add all the topics that need to happen (just like in reset topic)? It's ard to tell if any of the below happened above.

. Use the configuration wizard to complete the initial switch setup. (I assume this popped up when the new switch got powered on.)
+
...
Lisa the above text assumes everything else in the switch setup was already done...only CSHM is left to do. Read through to see if it was...if not, accommodate as needed. Maybe I don't need any of these last steps I added Apr 30 #7-12.
