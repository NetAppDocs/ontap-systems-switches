...
. Lisa does it make sense to add all the topics that need to happen (just like in reset topic)? It's hard to tell if any of the below happened above.
...
After you've replaced the switch, you link:../switch-cshm/config-overview.html[configure switch health monitoring].

Lisa the above text assumes everything else in the switch setup was already done...only CSHM is left to do. Read through to see if it was...if not, keep the sw setup steps as shown (repeated from what's used in reset topic). May 8.
