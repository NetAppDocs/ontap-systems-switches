...
* If you are installing or upgrading the NX-OS on a switch part of a switch replacement or resetting a switch to factory default settings (in an AFX system that is up and running), verify that your cluster is fully functional, meaning that there are no errors in the logs or similar issues.
+
If you are installing or upgrading the NX-OS as part of a new AFX system installation, you can skip this requirement.
...
. Lisa these two steps that follow are the first 2 steps from the P, Prepare to install NXOS and RCF topic, steps section. How do these apply to the 3 use cases?:
.  Verify that the version of NX-OS on your switch(es) is the current NetApp-supported version.
+
...
You install NX-OS on one of the switches (cs1) and then you repeat these steps to install NX-OS on the second switch (cs2).
...
== Step 3: Complete the NX-OS installation

Complete the NX-OS installation on both switches by verifying that the cluster LIFs have reverted to their home ports and then enable auto-revert on the cluster LIFs.
...
After you've installed NX-OS on your switches, you install RCF starting with the link:configure-install-upgrade-rcf-overview-9332d.html[Install or upgrade the RCF overview].
