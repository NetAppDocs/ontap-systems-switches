...
summary: After you install Cisco Nexus 9332D-GX2B switches as part of your AFX system installation or a switch replacement, you need to configure the switch(es).
---
= Initial switch setup - Cisco Nexus 9332D-GX2B
:icons: font
:imagesdir: ../media/

[.lead]
After you power on Cisco Nexus 9332D-GX2B switch(es) as part of a new AFX system installation or a switch replacement, or after you reset a switch to its factory default settings, you are prompted to complete the initial setup for the switch(es) using configuration wizard. You can reference the configuration wizard questions and recommended responses in this topic to help you complete the initial setup of the switch(es).

.Before you begin

* If you are installing switches as part of a new AFX system installation, make sure you have completed the https://docs.netapp.com/us-en/ontap-afx/install-setup/power-on-configure-switch.html[Power on and configure the switch] procedure for each switch in your cluster (AFX system?).
+
Lisa: note that the above topic in the AFX repo links back to my Configure sw and RCF workflow topic, which has users complete switch config, sw, RCF, SSH, health monitor and SCH, THEN links back to AFX repo to pwr on the system. Hey, in what switch topic does it link back to the AFX repo?\

* Make sure you have the applicable licenses, network and configuration information, and cables.
* Make sure you have access to link:required-documentation-9332d-cluster.html[Required switch and ONTAP documentation].
//* The switch(es) must be powered on...
...
.About this task
When you power on the switch(es), they begin to boot and you are prompted to provide answers to a series of questions to perform the initial (startup) configuration of the switch(es).

.Steps
//. Power on the switch(es).
//+
//They begin to boot.

. Perform the initial (startup) configuration of the switch(es) by providing responses to the following startup questions.
+
...
After you've configured your switches, you link:install-nxos-overview-9332d-cluster.html[prepare to install the NX-OS software and RCF].
