...
summary: You upgrade the Reference Configuration File (RCF) version on your Cisco Nexus 9332D-GX2B switches to support new hardware, a new ONTAP version, or to align with a new NX-OS version in your AFX 1K or AFX 2K system configuration.
---
= Upgrade the RCF version - Cisco Nexus 9332D-GX2B
:icons: font
:imagesdir: ../media/

[.lead]
You upgrade the Reference Configuration File (RCF) version on your Cisco Nexus 9332D-GX2B switches to support new hardware, a new ONTAP version, or to align with a new NX-OS version in your AFX 1K or AFX 2K system configuration.

//You upgrade the Reference Configuration File (RCF) version on your Cisco Nexus 9332D-GX2B switches when a version is available that supports new hardware, a new ONTAP version, or new features that you want to use in your AFX system.

//Use cases for upgrading RCF: 

//Upgrading the Reference Configuration File (RCF) on NetApp Cisco switches ensures compatibility when introducing new hardware, upgrading to a new ONTAP version that requires a different RCF, or enabling enhanced features like new breakout ports. It is primarily done to maintain cluster stability and support updated switch hardware configurations. 

//Key Reasons to Upgrade RCF Version:

//* New Hardware Support: You are adding new storage controllers (nodes) or storage shelves that require different port configurations.
    
// *ONTAP Upgrades: The target NetApp ONTAP version requires an updated RCF to properly manage switch networking.
    
// *NX-OS Updates: While a routine NX-OS upgrade doesn't always require an RCF change, a significant update may require an updated RCF for optimal performance.

// *Switch Migration/Expansion: Migrating from older switches (like the CN1610) to new ones or changing from a 2-node to a larger switch-based cluster.
...
* Save a copy of the current switch configuration.
//see step 2: step 4 and 5: save a copy of the current switch configuration. But isn't the bullet above saying this too?

* You must have a fully functioning AXF system (no errors in the logs or similar issues).

* You must have the current RCF version.
//You mean should have downloaded a copy of the current RCF version from the NetApp support site before starting the upgrade? 

* You need a boot configuration in the RCF that reflects the desired boot images. 
//What's this mean? That first bit is not needed since THIS IS the upgrade RCF topic.
+
...
* Before installing a new NX-OS version and RCF version, you must erase the switch settings and perform basic configuration. You must be connected to the switch using the serial console or have preserved basic configuration information prior to erasing the switch settings.
+
HEY, the above bullet is in today's 9332 Update RCF topic. The way it's worded sounds like it ONLY applies to installing NX-OS and installing RCF. Is this "requirement" needed in this upgrade RCF topic and/or in installing NX-OS and installing RCF?


.About this task
* The type of AFX system your switches are in determines the supported versions of RCF for your switches.

** For switches in an AFX 1K system, running any supported version of ONTAP, RCF 10.0 and later is supported.

** For switches in an AFX 2K system, running ONTAP 9.19.1GA or later, RCF 10.1 and later is supported.
+
...
Refer to the Knowledge Base article https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Node_out_of_quorum_when_migrating_cluster_lif_during_switch_OS_upgrade[Node out of quorum when migrating cluster LIF during switch OS upgrade^] for further details. 
...
//Need to keep 10.2 reference for 1K users running an earlier version?

//May 18: AD says to kill 4a and reword 4b: Use the show diff running-config command to compare with the saved RCF file in the bootflash. Otherwise, use a third party diff/compare tool.
//+
//However, what's the "otherwise" statement referring to?
...
+
//This is from install RCF (it's all new, wasn't there in original 9332 topic):
...
//This step already existed in original 9332 topic...which is weird since a similar step is in the install rcf.
+
If you have an AFX 2K system (which supports RCF 10.1 and later), you can link:https://review.docs.netapp.com/us-en/ontap-systems-switches_hotfix-refactor-phase1/switch-cisco-9332d-gx2b/install-upgrade-rcf-overview-cluster.html#change-port-speed-configuration-for-afx-a2k-systems[change the port speed configuration] for RCF 10.1 as needed.
...
. Repeat steps 4 to 18 on switch cs2.
...
After you've upgraded the RCF on your switches, you link:configure-ssh-keys.html[verify SSH and SSH keys are enabled].

//However, if your switches are running RCF 10.1 (only supported in an AFX 2K system), you can link:change-port-speed.html[change the default port speed configuration] if needed, and then you link:configure-ssh-keys.html[verify SSH and SSH keys are enabled].
