...
[.lead]
You upgrade the RCF version on your switches when a version is available that supports new hardware, a new ONTAP version, or new features that you want to use.

Note: Apr 25 Lisa: I believe I also need the option tab/some way to talk about the two different version of RCF like in the install RCF topic: If you're using 9223/9364 on 1K system, you use RCF version 10.0 (like today). If using these switches on 2K, you use the new RCF version 10.1.

NO!!! 1K with 10.0 was the only version! There was nothing to upgrade to. That was one of my questions I wanted to ask - how come you have an RCF upgrade topic  but there's no other version of RCF??? Was it the same for the NX-OS - just one version?

//Use cases for upgrading RCF: 

//Upgrading the Reference Configuration File (RCF) on NetApp Cisco switches ensures compatibility when introducing new hardware (e.g., AFF A320/A250), upgrading to a new ONTAP version that requires a different RCF, or enabling enhanced features like new breakout ports. It is primarily done to maintain cluster stability and support updated switch hardware configurations. 
...
.About this task
You must upgrade the RCF on both of the switches. This procedure has you upgrade the RCF on the first switch (cs1) and then has you repeat the steps for the second switch (cs2).
