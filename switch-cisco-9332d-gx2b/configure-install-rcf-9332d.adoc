...
//* Hey: this step only belongs in install RCF...and Upgrade? Seems to be covered in steps 1 and 2. 
...
NOTE: New switches do not ship with the RCF installed. You must install the RCF on the switches.
+
// Above 2 steps (HTTP and dnld) were in this order in original/pre-refactored topic Configure the 9332 switch (config wizard topic) and so applicable to new installs...I think.
+
...
//+
//Above is for install NXOS. But doesn't exist in today's install RCF.
//+
//The following ping step is for install NXOS. But doesn't exist in today's install RCF.
