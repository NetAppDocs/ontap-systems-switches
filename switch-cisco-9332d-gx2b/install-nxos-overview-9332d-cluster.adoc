...
---
= Prepare to install NX-OS software and RCF - Cisco Nexus 9332D-GX2B

:icons: font
:imagesdir: ../media/

[.lead]
If you installed new Cisco Nexus 9332D-GX2B switch(es) as part of a new AFX system installation, a switch replacement, or you have an existing switch on which you reset settings to the factory defaults, you must prepare your switch(es) and AFX system before you install the NX-OS and RCF on the switch(es).

//If you are installing a new switch as part of a new AFX system installation, a switch replacement, or you have an existing switch on which you need to reset the settings to factory defaults, you must prepare your switches and AFX system before you install the NX-OS software and RCF on the Cisco Nexus 9332D-GX2B switch(es).

Preparing your switches and AFX system includes downloading the current/supported version of software if needed, downloading the RCF releases, suppressing automatic case creation, checking the status of each port, verifying the connectivity of the remote cluster interfaces, and verifying that the auto-revert command is enabled on all cluster LIFs.

//Before you install the NX-OS software and the Reference Configuration File (RCF) on the Cisco Nexus 9332D-GX2B switch(es), you prepare by downloading the current/supported version of software if needed, downloading the RCF releases, suppressing automatic case creation, checking the status of each port, verifying the connectivity of the remote cluster interfaces, and verifying that the auto-revert command is enabled on all cluster LIFs.

.Before you begin
* The switches must be powered on and you completed the switch configuration (as needed) by following the procedure in link:setup-switch-9332d-cluster.html[Configure the 9332D-GX2B switch].

* You must have access to an HTTP, FTP, or TFTP server. Access to one of these servers allows you to download the NX-OS software (if needed) and Reference Configuration File (RCF) releases as instructed later in this procedure
...
.  Verify that the version of software on your switch(es) is the supported 10.4.2 version.
+
If it is not, download the NetApp-supported version of the software to the switches from the https://software.cisco.com/download/home[Cisco software download^] page.
//+
//NOTE: New switches should ship with the 10.4.2 version of the NX-OS software.
+
====
NOTE: 

NX-OS 10.4.2 is the only supported version for Cisco Nexus 9332D-GX2B switches in an AFX system.

Do not upgrade or downgrade your NX-OS version to an unsupported version.
====
+
. Download the applicable NetApp shared network and management network RCFs from the NetApp Support Site at http://mysupport.netapp.com/[mysupport.netapp.com^]. 
+
...
After you've prepared to install the NX-OS software and RCF, you link:install-nxos-software-9332d-cluster.html[install or upgrade the NX-OS software].
