...
summary: "Upgrade the NX-OS on Cisco Nexus 9332D-GX2B switches to support new hardware or a new ONTAP version on AFX 1K or AFX 2K systems."
---
= Upgrade NX-OS on the Cisco Nexus 9332D-GX2B switch
:icons: font
:imagesdir: ../media/

[.lead]
Upgrade the NX-OS on Cisco Nexus 9332D-GX2B switches to support new hardware or a new ONTAP version on AFX 1K or AFX 2K systems.

NOTE: NetApp recommends that you complete any ONTAP software upgrade first, then perform the software upgrade for your switches.
...
* If needed, you can refer to the link:https://www.cisco.com/c/dam/en/us/td/docs/dcn/tools/nexus-9k3k-issu-matrix/index.html[Cisco Nexus 9000 and 3000 Upgrade and ISSU Matrix^], which provides information on In-Service Software Upgrade (ISSU) for Cisco Nexus 9000 Series Switches based on your current and target releases.
+
...
You install the NX-OS on the first switch (cs1) by installing and verifying the new versions of the NX-OS and EPLD, verifying the health of all ports on the cluster, and then repeating these steps on the second switch (cs2).

//This step and tabs below are from 9336 Upgrade NX-OS.
...
//Lisa, you need to replace these steps with what's in 9808 Upgrade NX-OS.
...
. Old (original) steps below. I think steps 1-6 below get replaced bu the tabbed step above. But rest of steps (below) after that for to verify ONTAP stuff, so should stay. But check against 9336 Upgrafe NX-OS steps to make sure.
...
. Using the switch CLI, upgrade the EPLD image using the following commands:
+
`show version module 1 epld`
+
...
. After the switch reboots, use the switch CLI to log in and verify that the new version of EPLD loaded successfully.
+
`show version module 1 epld`
+
.Show example
