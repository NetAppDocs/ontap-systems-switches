...
//* You must have access to an HTTP, FTP, or TFTP server. Access to one of these servers allows you to download a supported version of NX-OS and RCF as instructed later in this procedure. Hey this is not in the prepare to install NX-OS and RCF topic (that's being removed). So I think it only applies to Upgrades.
