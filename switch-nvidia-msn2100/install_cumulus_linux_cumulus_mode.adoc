...
.Steps
. Log in to the switch.  First time login to the switch requires username/password of *cumulus*/*cumulus* with sudo privileges:
+
----
...
+
----
cumulus@cumulus:mgmt:~$ net add hostname sw1     cumulus@cumulus:mgmt:~$ net add interface eth0 ip address 10.233.204.71/23
