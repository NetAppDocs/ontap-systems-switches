...
summary: "Install the Ethernet switch health monitor (CSHM) configuration file for NVIDIA SN2100 storage switches to enable switch health monitoring in ONTAP."
---
= Install the Ethernet switch health monitor configuration file for NVIDIA SN2100 storage switches
:icons: font
:imagesdir: ../media/

[.lead]
Install the Ethernet switch health monitor (CSHM) configuration file for NVIDIA SN2100 storage switches to enable switch health monitoring in ONTAP. 
...
. Run the command `system switch ethernet configure-health-monitor show` on the ONTAP system and make sure that the cluster switches are discovered with the monitored field set to *True* and the serial number field not showing *Unknown*.
+
[subs=+quotes]
----
