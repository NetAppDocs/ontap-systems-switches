...
summary: "Install or upgrade the reference configuration file (RCF) script on NVIDIA SN2100 storage switches."
---
= Install or upgrade the RCF script on NVIDIA SN2100 storage switches
:icons: font
:imagesdir: ../media/

[.lead]
Install or upgrade the reference configuration file (RCF) script on NVIDIA SN2100 storage switches to configure port assignments and network settings.
...
----
. Apply the RCF python script *MSN2100-RCF-v1.8-Cluster*:
+
[subs=+quotes]
----
...
. Reapply any previous customizations to the switch configuration. Refer to link:cabling-considerations-sn2100-storage.html[Review cabling and configuration considerations] for details of any further changes required.
...
.What's next?
After you've installed or upgraded the RCF, you can link:setup-install-cshm-file.html[install the CSHM file].
