---
permalink: switch-nvidia-sn2100/migrate-cisco-sn2100-storage-switch.html
sidebar: sidebar
...
. The storage ports on each node are connected to existing storage switches in the following way (from the nodes' perspective) using the command: `network device-discovery show -protocol lldp`
+
[subs=+quotes]
----
