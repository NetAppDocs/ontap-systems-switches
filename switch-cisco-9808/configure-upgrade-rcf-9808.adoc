...
summary: "Upgrade the RCF on Cisco Nexus 9808 switches to support new hardware, a new ONTAP version, or a new NX-OS version on AFX 2K systems."
---
= Upgrade the RCF on the Cisco Nexus 9808 switch
:icons: font
:imagesdir: ../media/

[.lead]
Upgrade the reference configuration file (RCF) on Cisco Nexus 9808 switches to support new hardware, a new ONTAP version, or a new NX-OS version on AFX 2K systems.
...
* The examples shown in this procedure are for AFX 2K systems with switches running RCF 10.1. The input and output can vary depending on your RCF version.
...
`move bootflash:write_erase.cfg write_erase_old.cfg`
====
+
`show run | i "username admin password" > bootflash:write_erase.cfg`
+
`show run | section "vrf context management" >> bootflash:write_erase.cfg`
+
`show run | section "interface mgmt0" >> bootflash:write_erase.cfg`
+
`show run | section "switchname" >> bootflash:write_erase.cfg`
+ 
...
. Using the switch CLI, erase the current saved configuration:
+
`write erase`
+
...
+
For more information on Cisco commands, refer to the appropriate guide in the link:https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.html[Cisco Nexus 9000 Series NX-OS Command Reference^].
+
...
. Using the switch CLI, verify that the RCF is the correct newer version: 
+
`show running-config`
+
