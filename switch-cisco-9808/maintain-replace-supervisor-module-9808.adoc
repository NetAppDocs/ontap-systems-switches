...
[.lead]
Follow these steps to replace a defective Cisco Nexus 9808 supervisor module in a cluster network. This is a nondisruptive procedure (NDU)
...
====
+
If the supervisor module that you are replacing is not functional and is powered down, go to Step 6. The LIFs on the cluster nodes should have already failed over to the other cluster port for each node.
