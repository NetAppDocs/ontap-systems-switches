...
* RCF 10.1 is supported on both AFX 1K and AFX 2K platforms. For your AFX 2K system, RCF 10.1 and later is supported.

* RCF 10.1 has different files for different use cases. Depending on how you plan to use the switches in your AFX 2K system configuration (shared, storage only, or cluster and HA only), you choose the applicable RCF file.
+
For example, if you are using your switches as shared switches in an AFX 2K system, you use the RCF file appropriate for that use case. The appropriate RCF file could be named similarly to `RCF-v10.1-NX9808-CL1-ST1.txt`. The following table shows the preset groups of the switch ports, their port speeds, and usage for this RCF file.
...
* Refer to the https://hwu.netapp.com/SWITCH/INDEX[NetApp Hardware Universe^] to verify the correct ports on your platform.
