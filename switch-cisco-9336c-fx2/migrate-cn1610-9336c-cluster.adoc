...
summary: "Migrate NetApp CN1610 cluster switches to Cisco Nexus 9336C-FX2 and 9336C-FX2-T cluster switches without disrupting cluster operation."
---
= Migrate from NetApp CN1610 cluster switches to Cisco Nexus 9336C-FX2 and 9336C-FX2-T cluster switches
:icons: font
:imagesdir: ../media/

[.lead]
Migrate existing NetApp CN1610 cluster switches to Cisco Nexus 9336C-FX2 and 9336C-FX2-T cluster switches without disrupting cluster operation.
...
For details of supported ports and their configurations, refer to the link:https://hwu.netapp.com/[Hardware Universe^]. Refer to link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU[What additional information do I need to install my equipment that is not in HWU?^] for more information about switch installation requirements.
...
. On switch cs2, shut down and restart all cluster ports to trigger an auto-revert of all cluster LIFs that are not on their home ports.
+
...
`network interface show -vserver Cluster`
+
If any of the cluster LIFs have not reverted to their home port, manually revert them. You must connect to each node-mgmt LIF or SP/BMC system console of the local node that owns the LIF: 
+
