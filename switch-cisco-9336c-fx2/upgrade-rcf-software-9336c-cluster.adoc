...
summary: "Upgrade the reference configuration file (RCF) on Cisco Nexus 9336C-FX2 and 9336C-FX2-T cluster switches when a newer version is available."
---
= Upgrade the RCF on Cisco Nexus 9336C-FX2 and 9336C-FX2-T cluster switches
:icons: font
:imagesdir: ../media/

[.lead]
Upgrade the reference configuration file (RCF) on Cisco Nexus 9336C-FX2 and 9336C-FX2-T cluster switches when a newer version of the RCF is available. 
...
. Copy the RCF to the bootflash on switch cs1 using one of the following transfer protocols: FTP, SFTP, or SCP. 
+
For more information on Cisco commands, refer to the appropriate guide in the link:https://www.cisco.com/c/en/us/support/switches/nexus-9336c-fx2-switch/model.html#CommandReferences[Cisco Nexus 9000 Series NX-OS Command Reference^].
+
This example shows SFTP being used to copy an RCF to the bootflash on switch cs1:
+
...
CAUTION: If your 9336C-FX2 switch fails to boot after your RCF upgrade, refer to the NetApp Knowledge Base article link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Cisco_9336C-FX2_Switch_Fails_to_Boot_After_RCF_Install_%E2%80%93_Boot_Variable_Cleared[Cisco 9336C-FX2 Switch Fails to Boot After RCF Install – Boot Variable Cleared^] for next steps.
...
*NOTE:* Wait for a number of seconds before running the `show` command to display the details. Check the entries matching today's date.
...
// GH issue #303, 2025-OCT-28
// AFFFASDOC-456, 2026-JAN-30
// AFFFASDOC-473, 2026-MAR-18
// AFFFASDOC-479, 2026-APR-13